...
func init() {
	filter = NewBloomFilter(3, 100)
}

func AccountBloomFilter(ctx *context.Context) {
	var requestBody map[string]interface{}
	json.Unmarshal(ctx.Input.RequestBody, &requestBody)
	log.Println(requestBody)
	id := requestBody["id"]
	if id != nil {
		if filter.Check([]byte(strconv.Itoa(id.(int)))) {
			denyRequest(ctx, "already exists")
			return
		} else {
			filter.Add([]byte(strconv.Itoa(id.(int))))
		}
	}
}
